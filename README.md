...
[官网 / Website](https://swy2018.github.io/Aurora-Audio-Studio/)
...
| 去人声 / 分轨 | BS-RoFormer | 纯人声 Vocal + 纯伴奏 Instrumental |
| AI 扒谱 | Basic Pitch | MIDI 和音符事件表 |
| 视频字幕 | Subtitle Edit | SRT 字幕和时间轴校对 |

### 路线图

- `0.8.x`：稳定本地启动器、模型安装入口、输出目录整理。
- `0.9.x`：更完整的模型管理、安装状态检测、错误恢复提示。
- `1.0.0`：正式安装版、开始菜单图标、卸载流程、自动更新。
- 未来：插件系统、社区模型目录、批量任务、歌词辅助、更多 DAW/字幕工作流。

### 扩展模型目录

Aurora 内置默认模型入口，同时支持可选的 `models.json` 扩展目录。普通用户不需要关心它；开发者或社区贡献者可以参考：

```text
work/audio-studio/models.example.json
```

如果要在不改源码的情况下测试新模型入口，可以把自己的 `models.json` 放到：

```text
%LOCALAPPDATA%\Aurora Audio Studio\models.json
```

这样不会污染桌面，也不会在 exe 同目录生成配置文件。
...
| Workflow | Default entry | Best for |
|---|---|---|
| Music generation | ACE-Step 1.5 | Songs, backing tracks, melody drafts |
| AI voice | IndexTTS2 | Narration, dialogue, speaking voice cloning |
| Singing voice clone | Seed-VC 44.1k | Singing timbre conversion |
| Vocal removal / stems | BS-RoFormer | Vocal and instrumental outputs |
| MIDI transcription | Basic Pitch | MIDI and note-event extraction |
| Video subtitles | Subtitle Edit | SRT subtitles and timing correction |

### Roadmap

- `0.8.x`: stable local launcher, model install entries, organized outputs.
- `0.9.x`: stronger model management, install-state detection, and error recovery.
- `1.0.0`: formal installer, Start Menu entry, uninstall flow, and update checks.
- Future: plugin system, community model catalog, batch jobs, lyric helpers, and more DAW/subtitle workflows.

### Extensible model catalog

Aurora includes default model entries and also supports an optional external `models.json` catalog. Normal users do not need it; developers and community contributors can start from:

```text
work/audio-studio/models.example.json
```

For local testing without changing source code, place a custom catalog here:

```text
%LOCALAPPDATA%\Aurora Audio Studio\models.json
```

This keeps user configuration out of the desktop and out of the executable folder.
