...
public sealed record ModelInstallPlan(string TargetPath, string EstimatedDownload, string RecommendedFreeSpace);

public static class ModelInstallPlanner
{
    public static ModelInstallPlan Create(ModelDefinition model, string modelRoot) => new(
        Path.GetFullPath(Path.Combine(modelRoot, model.RelativeRoot)),
        EstimatedDownload(model.Id),
        RecommendedFreeSpace(model.Id));
