    ...
    public async Task<int> ImportAsync()
    {
        ...
        try
        {
            ...
            foreach (var receiptPath in Directory.EnumerateFiles(root, "*.json"))
            {
                ...
                var taskId = Guid.TryParseExact(receipt.TaskId, "N", out _) ? receipt.TaskId : null;
                var projectId = taskId ?? receipt.Id;
                var projectPath = Path.Combine(settings.Current.ProjectsRoot, projectId + ".arr");
                var existing = File.Exists(projectPath) ? projects.Find(projectId) : null;
                var task = queue.Items.FirstOrDefault(x => taskId is not null ? x.Id == taskId : x.ProjectId == projectId);
                ...
                queue.RegisterCompleted(task, project.Artifacts.Select(x => x.Path).ToArray());
