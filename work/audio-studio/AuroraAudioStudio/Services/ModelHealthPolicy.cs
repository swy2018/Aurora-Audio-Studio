...
public static class ModelHealthPolicy
{
    ...
    {
        var root = deploymentRoot ?? Path.Combine(localAiRoot, model.RelativeRoot);
        if (model.UpdateKind == "uv-package") root = RuntimeEnvironment.Resolve(root);
        ...
        {
            case "ace-step":
                if (!File.Exists(Path.Combine(root, "python_embeded", "python.exe")) && !File.Exists(Path.Combine(RuntimeEnvironment.Resolve(Path.Combine(root, ".venv")), "Scripts", "python.exe")))
            ...
            case "transkun":
                ...
                break;
            case "roformer":
                required.Add(("BS-RoFormer-SW 权重", Path.Combine(localAiRoot, "AudioTools", "roformer-models", "roformer-model-bs-roformer-sw-by-jarredou", "BS-Rofo-SW-Fixed.ckpt")));
                required.Add(("BS-RoFormer-SW 配置", Path.Combine(localAiRoot, "AudioTools", "roformer-models", "roformer-model-bs-roformer-sw-by-jarredou", "BS-Rofo-SW-Fixed.yaml")));
                break;
            case "roformer-vocals":
                required.Add(("分轨共享运行环境", Path.Combine(RuntimeEnvironment.Resolve(Path.Combine(localAiRoot, "AudioTools", "roformer-env")), "Scripts", "bs-roformer-infer.exe")));
                break;
            case "piano":
            ...
            case "qwen3-tts-06b-custom":
                required.Add(("Qwen3-TTS 启动器", Path.Combine(RuntimeEnvironment.Resolve(Path.Combine(localAiRoot, "Qwen3-TTS", "Python312")), "Scripts", "qwen-tts-demo.exe")));
                break;
            case "whisper-small":
            ...
            case "whisper-large-v3":
                ...
                break;
            case "minimax-music3":
                required.Add(("MiniMax Python 运行环境", Path.Combine(RuntimeEnvironment.Resolve(Path.Combine(localAiRoot, "AudioTools", "minimax-music3-env")), "Scripts", "python.exe")));
                break;
        }
