# Aurora Audio Studio 1.7.0 使用说明
...
Aurora 1.7.0 重点修复任务取消与模型部署链路：已取消的排队任务不会再启动，取消一个任务不会误停其他引擎；模型下载使用独立环境，ACE-Step 与 Seed-VC 可由 Aurora 完整配置。安装页会保留清晰阶段和最近日志，设置、素材格式、模型运行环境与处理结果都会在写入前做更严格的检查。
