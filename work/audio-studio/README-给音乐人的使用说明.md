# Aurora Audio Studio 1.6.0 使用说明
...
Aurora 1.6.0 将模型管理按六个功能分组。GitHub 模型优先检测正式 Release；没有正式 Release 时显示默认分支最新提交的日期版，Hugging Face 模型显示官方更新时间并比较精确快照。新增 HeartMuLa、IndexTTS 2.5、SoulX-Singer SVC、Qwen3-ASR 与 Qwen3-ForcedAligner 管理入口，但 Aurora 仍不会在启动时自行下载模型，只有用户主动操作才会安装。
