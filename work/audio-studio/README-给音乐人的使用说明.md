# Aurora Audio Studio 1.5.1 使用说明
...
Aurora 1.5.1 在任务工作台选择未安装模型时会显示独立的“下载安装模型”按钮，安装完成后才切换为“进入工作台”。组件断点续传遇到 HTTP 416 时会先核对官方文件大小，再继续 SHA-256 完整性校验，不再把已经完整下载的包误报为失败。Aurora 仍不会在启动时自行下载模型。
