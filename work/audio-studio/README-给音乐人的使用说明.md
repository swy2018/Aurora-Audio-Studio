# Aurora Audio Studio · Windows 使用说明

Aurora 将音乐创作、配音、歌声转换、分轨、MIDI 扒谱和字幕制作整合到一个本地工作台。选择功能、准备素材并安装所需模型，即可开始创作。

处理记录保存素材位置、所选模型、参数和输出文件，便于再次处理。记录本身不包含素材或模型，也不是音频编辑工程；迁移到另一台电脑时，请同时保留对应素材。旧版 `.aurora` 记录仍可读取。
...
模型中心会区分文件齐全与运行验证状态。建议先处理一段短素材，确认模型与当前设备兼容，再处理完整作品。
...
6. 选择未安装的创作模型时，点击“安装模型”；确认目标目录、预计下载量和建议预留空间后才会下载或部署。
...
字幕生成后，可在成品库检查时间轴并导出。需要其他功能时，再到模型中心安装对应模型。
...
- 点击“结束当前引擎”会停止当前模型及其未完成任务，并释放占用资源。请先保存需要保留的结果。
- Aurora 不开机自启动。
...
在“关于”中选择“回退到上一个正式版”，可查看适用于 Windows 的最近较早正式版，确认后下载、校验并安装。正式版和 Beta 均可使用，更新通道保持不变。旧版可能无法读取较新配置或记录，请先备份重要数据。应用安装不会删除模型和成品；模型自身的备份与回退不受影响。
