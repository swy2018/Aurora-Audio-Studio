# Aurora Audio Studio 1.4.1 使用说明
...
Aurora 1.4.1 只允许一个应用实例同时操作任务、设置和模型；Hugging Face、Git、PyPI 与五个固定组件都可通过模型中心检查和更新。所有替换会先在暂存目录完成来源与完整性校验，再切换到正式目录并保留上一版本。选择未安装模型时，任务工作台会直接显示安装按钮，但 Aurora 不会在启动时自行下载模型。
