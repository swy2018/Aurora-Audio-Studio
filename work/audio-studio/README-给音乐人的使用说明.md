# Aurora Audio Studio 1.5.0 使用说明
...
Aurora 1.5.0 的模型中心使用更直观的分层卡片展示用途、语言、版本、许可、状态和本地路径。检查后可直接更新单个组件，也可按顺序更新全部；失败项会保留供重试。若 Subtitle Edit 等组件仍在运行，Aurora 会提示先保存并关闭，不会强制结束程序。选择未安装模型时，任务工作台仍会直接显示安装按钮，但 Aurora 不会在启动时自行下载模型。
