# Aurora Audio Studio 1.8.0 使用说明
...
Aurora 1.8.0 重点补齐六条工作流的真实可操作性：音乐、配音、歌声、分轨、扒谱与字幕均提供明确输入、参数、执行、进度、取消和结果入口。正式安装版的 WebView2 会在可加载状态下初始化，初始化与页面导航均有 30 秒超时和取消保护；ACE-Step 启动会检查进程、端口、超时与日志。GPU 可用时优先 CUDA，模型健康检查也会同时核对权重、运行时和关键依赖，不再只显示一个无法继续的框架。
