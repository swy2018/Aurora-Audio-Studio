# Aurora Audio Studio 1.8.1 使用说明
...
Aurora 1.8.1 修复 1.8.0 安装到 Program Files 后引擎显示“已连接”、中央工作区却空白的问题：WebView2 用户数据固定保存到可写的 `%LOCALAPPDATA%\Aurora Audio Studio\WebView2`，音乐、配音和歌声工作台可以真正显示。1.8.0 补齐的六条操作链路继续保留；音乐、配音、歌声、分轨、扒谱与字幕均有明确输入、参数、执行、进度、取消和结果入口，GPU 可用时优先 CUDA，模型健康检查同时核对权重、运行时和关键依赖。
