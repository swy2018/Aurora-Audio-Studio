# Aurora Audio Studio 1.6.1 使用说明
...
Aurora 1.6.1 修复了重复点击安装不同模型时多个部署任务同时写入环境、进度显示混在一起的问题。模型安装现在一次只执行一个；进度会标明正在安装的模型，取消也只作用于当前任务。已有安装运行时再次点击会明确提示等待，不会再启动第二个安装进程。
